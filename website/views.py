...
def home(request):
    lang_list = ['bash', 'c', 'clike', 'cpp', 'csharp', 'css', 'django', 'go', 'java', 'javascript', 'json', 'markdown', 'markup', 'markup-templating', 'python', 'shell-session']

    if request.method == "POST":
        code = request.POST['code']
        lang = request.POST['lang']
        #check to make sure user picked a lang
        if lang == "Select Programming Language":
            messages.success(request, "Hey, you forgot to select the language")
...
def login_user(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.success(request, "Error logged In, please correct your username and password! ")
            return redirect('home')
    else:
...
def logout_user(request):
    logout(request)
    messages.success(request, "You have been logged out! Have nice day! ")
    return redirect('home')
...
def register_user(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            login(request, user)
            messages.success(request, "You have registerred! Congrats...")
            return redirect('home')
